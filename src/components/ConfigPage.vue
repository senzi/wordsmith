	...
	<div class="config-page flex">
		<div class="w-full">
			...
			<section class="variables-table">
				...
				<div class="text-center">
					<button @click="clearAllConfig"
						class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-md mt-4">清空所有配置</button>
				</div>
						...
						<div class="card-body">
							...
							<ul class="list-none">
								<li v-for="(who, index) in whos" :key="index" class="flex items-center mb-2">
									...
									<button @click="removeWho(index)"
										class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-md">删除</button>
								</li>
							</ul>
							<button @click="addWho"
								class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-md mx-auto">添加
								Who</button>
						</div>
					</div>
						...
						<div class="card-body">
							...
							<ul class="list-none">
								<li v-for="(where, index) in wheres" :key="index" class="flex items-center mb-2">
									...
									<button @click="removeWhere(index)"
										class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-md">删除</button>
								</li>
							</ul>
							<button @click="addWhere"
								class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-md mx-auto">添加
								Where</button>
						</div>
					</div>
		...
		<div class="config-tools w-1/2 pl-4">
			...
			<div v-if="showImport">
				...
				<button @click="importFromBase64"
					class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">使用文件导入</button>
				<textarea v-model="base64Config" placeholder="粘贴 base64 配置文本"
					class="border p-2 w-full h-48 mt-4"></textarea>
				<button @click="importFromBase64"
					class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded mt-4">文本配置导入</button>
			</div>
			<div v-if="showExport">
				<h2 class="text-lg font-bold mb-4">导出配置</h2>
				<!-- 复制配置按钮 -->
				<button @click="copyConfigToClipboard"
					class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">复制配置</button>
				<!-- 下载 txt 文件按钮 -->
				<button @click="exportToTxt"
					class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded ml-4">下载 txt 文件</button>
				<!-- 配置文本的 textarea -->
				<textarea readonly :value="exportedConfig" class="border p-2 w-full h-48 mt-4"></textarea>
			</div>
			<div class="flex mt-4">
				<button @click="toggleImport"
					class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded mr-2">导入</button>
				<button @click="toggleExport"
					class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded">导出</button>
			</div>
		</div>
	</div>
</template>
