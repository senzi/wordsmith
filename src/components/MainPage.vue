  ...
  <div class="flex flex-col h-screen justify-between">
    ...
    <main class="flex-grow container mx-auto p-4">
      ...
      <div class="text-center">
        <button @click="generateSentence" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded my-4">
          点击生成
        </button>
      </div>
      <!-- History Card -->
      <section class="history my-6 mx-auto p-4 max-w-xl bg-white rounded-lg shadow-md">
        <ul class="list-none text-center text-lg">
			...
			<li v-for="(item, index) in history.slice(0, 10)" :key="index"
			  ...
			  <div :class="{'bg-blue-100': index === 0, 'flex-grow': true, 'rounded-md': index === 0, 'py-2': index === 0, 'px-4': index === 0}">
				<!-- Timestamp tag -->
				<span class="timestamp-tag bg-gray-200 text-gray-700 rounded-full px-3 py-1 text-xs font-semibold mr-2">
				  {{ item.timestamp }}
				</span>
				{{ item.sentence }}
			  </div>
...
.new-tag {
  ...
  width: 50px; /* Adjust to match the NEW tag width with padding */
  height: 20px; /* Adjust to match the NEW tag height */
  color: red; /* Text color, ensure high contrast against the background */
  text-align: center;
...
.tag-space {
  display: inline-block;
  width: 50px; /* Adjust the width as needed to match the NEW tag */
